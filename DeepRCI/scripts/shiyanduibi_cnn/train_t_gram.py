import numpy as np
import pandas as pd
from network_t_gram import CMM_NET
...
def main():
    path_train = r'E:\data\AAC\one_gram\t_gram.csv'
    path_test = r'E:\data\AAC\one_gram\t_gram_val.csv'

    feature_train, label_train = get_feature(path_train, 8000)
    feature_test, label_test = get_feature(path_test, 8000)
    ...
    for epoch in range(2000):
        ...
        if acc > 0.77:
            with open(r'H:\400\y_score_t_gram.txt', 'a') as w_obj:
                w_obj.write(str(acc))
                w_obj.write('\n')
