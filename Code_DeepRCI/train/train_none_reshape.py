...
def main():
    path_feature_0_train = r'H:\DeepRCI_data\None_reshape\train\0\\'
    path_feature_1_train = r'H:\DeepRCI_data\None_reshape\train\1\\'

    # path_feature_00_val = r'F:\data_xmm\validation\0\\'
    # path_feature_11_val = r'F:\data_xmm\validation\1\\'
    path_feature_0_val = r'H:\DeepRCI_data\None_reshape\validation\0\\'
    path_feature_1_val = r'H:\DeepRCI_data\None_reshape\validation\1\\'
    path_feature_000_val = r'H:\DeepRCI_data\None_reshape\validation\00\\'
    ...
    db_train = db_train.map(preprocess).shuffle(1000000).batch(1)


    db_val = tf.data.Dataset.from_tensor_slices((path_val, label_val))
    db_val = db_val.map(preprocess).batch(1)

    db_val_1 = tf.data.Dataset.from_tensor_slices((path_val_1, label_val_1))
    db_val_1 = db_val_1.map(preprocess).batch(1)
